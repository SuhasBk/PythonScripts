...
if sys.platform == 'linux':
    ROOT_DIR = '/mnt/Universe/tv_movies/movies'
elif sys.platform == 'darwin':
    ROOT_DIR = '/Users/gandalf/Movies'
else:
    ROOT_DIR = r'E:\tv_movies\movies'


def file_filter(
    x): return '.srt' not in x and '.txt' not in x and '.unwanted' not in x and '.parts' not in x


series_list = list(map(lambda x: os.path.join(
    ROOT_DIR, x), os.listdir(ROOT_DIR)))
